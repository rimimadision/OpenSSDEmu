...
void L2P_init_buffer() {
  ...
  for (u32 i = 0; i < BUFFER_NUMBER; i++) {
    list_add_tail(&buffer_list_ptr->buffer_entry[i].free_list_node,
                  &free_buf_list_head);
  }
...
u32 L2P_allocate_buf(u32 lpn) {
  ...
  if (TRUE == list_empty(&free_buf_list_head)) {
    ...
    buf =
        container_of((&free_buf_list_head)->next, typeof(*buf), free_list_node);
...
u32 get_hash_code(u32 key) { return key % HASH_TABLE_SIZE; }
...
buffer_entry *search_hash_table_entry(u32 key) {
  ...
  u32 hash_value = get_hash_code(key);
  list_for_each_entry(buf, &hash_table_ptr->hash_table[hash_value],
                      hash_table_node) {
    if (buf->lpn == key) {
      return buf;
    }
...
void init_hcmd_buf_table() {
  ...
  for (u32 i = 0; i < 256; i++) {
    hcmd_buf_table_ptr->table_entry[i].next =
        &hcmd_buf_table_ptr->table_entry[i];
    hcmd_buf_table_ptr->table_entry[i].prev =
        &hcmd_buf_table_ptr->table_entry[i];
...
void L2P_hcmd_add_buffer(u32 hcmd_id, u32 buffer_id, u32 lpn) {
  ...
  list_add_tail(&buf->hcmd_buf_list_node,
                &hcmd_buf_table_ptr->table_entry[hcmd_id]);
...
u32 L2P_hcmd_buf_get_lpn(u32 hcmd_id, u32 buf_offset) {
  ...
  list_for_each_entry(buf, &hcmd_buf_table_ptr->table_entry[hcmd_id],
                      hcmd_buf_list_node) {
...
u32 L2P_hcmd_get_buffer(u32 hcmd_index, u32 offset) {
  ...
  list_for_each_entry(buf, &hcmd_buf_table_ptr->table_entry[hcmd_index],
                      hcmd_buf_list_node) {
    if (offset <= tmp_cnt) {
      return buf->buf_id;
    }
