...
void FTL_nvme_req_polling()
{
    ...
    shm_index index;
    shm_cmd scmd;
    // EMU_log_println(LOG, "%d in polling now", c);
    for (i = 0; i < MAX_POLLING_NUM; i++)
    {
        ...
        if (hcmd != NULL)
        {
            ...
            {
                hcmd->opcode = HCE_WRITE;
            }
            else if (scmd.ops == SHM_READ_OPS)
            {
                hcmd->opcode = HCE_READ;
            }
            else
            {
                EMU_log_println(ERR, "get wrong cmd from shm");
            }
            hcmd->start_lsn = scmd.lpn;
            hcmd->req_num = scmd.size / HOST_BLOCK_SIZE;
            hcmd->send_node_cnt = 0;
            hcmd->cur_lsn = hcmd->start_lsn;

            hcmd->cur_cnt = 0;
            hcmd->cpl_cnt = 0;
            hcmd->is_searched = HCE_ISNOT_SEARCHED;
            // used for finding cmd slot in shm
            hcmd->cmdSlotTag = index;
            hcmd->emu_id = ++c;
