...
void BE_send_sq(u32 sq_index, hw_queue_entry *sq_set, int ch, int ce)
{
    ...
    //   EMU_log_println(LOG, "send sq of hcmd %d", HCL_get_hcmd_entry_addr(sq->sq_entry.hcmd_index)->emu_id);
...
void BE_read_log(u32 ch, u32 ce, u32 lun, u32 plane, u32 block, u32 page, u32 buf_addr)
{
    ...
    u32 flash_log_sz = ftell(fp);
    //   EMU_log_println(LOG, "sz %u", flash_log_sz);
    u32 _ch, _ce, _lun, _plane, _block, _page;

    for (int i = (flash_log_sz / LOG_LEN); i >= 0; i--)
    {
        fseek(fp, i * LOG_LEN, SEEK_SET);
