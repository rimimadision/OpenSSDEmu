...
.. code-block:: shell
    ...
    pip install /Documentation/requirements.txt
...
.. note:: 
    ...
    本项目输出的 ``pdf`` 中的中文使用的是free open source的 ``WenQuanYi Zen Hei``

.. note:: 

    本人使用的是 ``Ubuntu 20.04 LTS`` + ``aliyun mirror``
    可能因为系统和镜像不同，需要安装的tex工具会不一样，可以根据 ``make latexpdf`` 的输出自行安装需要的工具,
    此外，还需要安装一下自己缺少的字体，下面是本人的环境安装流程，可以参考一下

.. warning:: 

    强烈建议20分钟内没有 ``make`` 成功，来找 `本人 <xionghongkang2019@bupt.edu.cn>`_ 讨论一下,
    也正好完善一下这部分文档
...
.. code-block:: shell
    ...
    cd Documentation/source && mkdir nvme && cd nvme
    echo "NVMe 文档" > index.rst
    echo "==========" >> index.rst
...
.. code-block:: shell
    ...
    cd Documentation/ && vim index.rst

在index.rst末尾加入rst代码（也可以自己选择加在什么位置）
